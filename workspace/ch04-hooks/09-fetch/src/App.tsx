  ...
  */
  ...
  const fetchTodo = async (fetchParams: FetchParams) => {
    ...
    try {
      ...
      if( jsonRes.ok ) {
        setData(jsonRes);
        // 위 코드 useEffect() 없이 실행 시 무한 리렌더링 발생, 위험
      }else {
  ...
  // 컴포넌트가 mount 뒤 이후, Todo 목록 조회
  useEffect(()=> {
    console.log('mount 후 한번만 호출');
    const params = { url: '/todolist?delay=1500'}
    fetchTodo(params)
  },[]); // 빈 배열 전달해서 mount시 한번만 실행되도록 설정
